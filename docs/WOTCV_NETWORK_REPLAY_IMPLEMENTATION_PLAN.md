...
Surowy capture zostaje początkowo włączony wyłącznie dla strony DEV. Produkcja nie może zostać objęta network capture bez osobnej decyzji po analizie danych z DEV.
...
Jeżeli testy DEV pokażą, że pełne body powodują zbyt duże odpowiedzi endpointu replay, kolejnym krokiem będzie wydzielenie ciężkich payloadów do osobnego storage i ich lazy loading. Nie jest to część pierwszej implementacji.
...
Proponowane wartości początkowe DEV:
...
- wyraźne ostrzeżenie, że DEV V1 zapisuje surowe headers i body.
...
Podczas testów DEV trzeba mierzyć:

- średni rozmiar sesji bez networku,
- średni rozmiar sesji z networkiem,
- percentyl P95 i P99,
- udział request/response body w rozmiarze,
- czas pobierania replay,
- czas parsowania po stronie klienta,
- zużycie dysku ClickHouse/R2.
...
- należy rozważyć Web Worker dopiero po wynikach testów DEV.
...
- `dev` — ostatnia zatwierdzona wersja DEV,
- `sha-<pełny-lub-skrócony-commit>` — niezmienny tag konkretnego commita,
- opcjonalnie `vX.Y.Z-wotcv.N` — wydanie produkcyjne.

Nie wdrażać produkcji wyłącznie z ruchomego `latest`.
...
- opcjonalnie `release/*` — przygotowanie wydania.
...
Tworzymy PR do `master`, uruchamiamy CI i wdrażamy wynik na DEV przed mergem albo bezpośrednio po mergu, zależnie od pipeline’u.
...
5. build client,
6. push tagów SHA,
7. dla `master` opcjonalnie aktualizacja tagu `dev`,
8. zapis digestów obrazów w podsumowaniu joba.
...
- tracker forka zaczyna nagrywać natywny network na DEV,
...
Przed produkcją trzeba przeanalizować i ewentualnie zaktualizować:
...
Na etapie DEV używać kontrolowanych kont i danych testowych.
...
### PR 8 — DEV rollout i poprawki

- włączenie dla site DEV,
- pomiary,
...
14. Rozmiary sesji i czasy ładowania są zmierzone i zaakceptowane.
15. Rollback do poprzedniego obrazu jest przetestowany.
...
- network replay enabled tylko dla site DEV,
- kontrolowane konta,
- brak danych produkcyjnych,
- capture 100% sesji testowych,
...
Osobny projekt po analizie realnych payloadów DEV:
...
### Faza E — ewentualna produkcja

Dopiero po:

- wdrożeniu maskowania,
- przeglądzie bezpieczeństwa,
- przeglądzie prawnym/prywatności,
- ustaleniu retencji,
...
- V1 działa na DEV bez maskowania networku,

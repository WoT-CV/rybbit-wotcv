...
describe("timezone list", () => {
  it("leads with the system sentinel and preserves the Polish fork label", () => {
    expect(timezones[0]).toEqual({ value: "system", label: "Systemowa" });
    expect(getTimezoneLabel("system")).toBe("Systemowa");
  });

...
export function EditTraitsDialog(props: EditTraitsDialogProps) {
  return <EditTraitsDialogContent key={props.open ? "open" : "closed"} {...props} />;
}

function EditTraitsDialogContent({ userId, traits, open, onOpenChange }: EditTraitsDialogProps) {
  ...
  const [baseline, setBaseline] = useState<Record<string, unknown>>(() => traits ?? {});

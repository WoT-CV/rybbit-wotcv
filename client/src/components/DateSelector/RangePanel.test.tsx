...
const day = (label: string) => {
  const [month, date, year] = label.split("/").map(Number);
  const localized = new Date(year, month - 1, date).toLocaleDateString();
  return document.querySelector<HTMLButtonElement>(`[data-day="${localized}"]`)!;
};

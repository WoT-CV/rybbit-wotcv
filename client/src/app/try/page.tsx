...
function TryPageContent() {
  ...
  const canCreate =
    !isLoadingConfigs && !configError && configs?.capabilities?.unclaimedSites === true && !configs.disableSignup;
  ...
  }, [initialDomain, canCreate]);

  if ((configs && !canCreate) || configError) {
    return (
      <div className="mx-auto max-w-md p-8">
        <AuthError error={configError?.message ?? t("Anonymous site creation is disabled")} />
      </div>
    );
  }

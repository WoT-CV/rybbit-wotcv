...
describe("formatAnnotationDate", () => {
  it("respects Polish date formatting without changing the instant or timezone", () => {
    Settings.defaultLocale = "pl-PL";
    expect(formatAnnotationDate(base, "America/Los_Angeles")).toBe("18 sie 2026");
    expect(formatAnnotationDate(base, "UTC")).toBe("18 sie 2026, 07:00");
  });
...
describe("parseAnnotationInstant", () => {
  it("reads ISO 8601 and Postgres timestamptz text alike", () => {
    ...
    expect(formatAnnotationDate({ ...base, date: "2026-08-18 07:00:00+00" }, "America/Los_Angeles")).toBe(
      "Aug 18, 2026"
    );
  });
});

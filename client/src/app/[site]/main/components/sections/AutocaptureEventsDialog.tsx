...
export function AutocaptureEventsDialogBody({
  events,
  valueLabel,
  countLabel,
  lastLabel,
}: AutocaptureEventsDialogBodyProps) {
      ...
      const observer = new IntersectionObserver(
        entries => {
          if (entries[0]?.isIntersecting) {
            setVisibleCount(count => Math.min(count + BATCH_SIZE, filteredAll.length));
          }
        },

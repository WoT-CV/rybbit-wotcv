      ...
      <CardContent className="p-2 md:p-4 py-3 w-full">
        <div className="flex items-center justify-between px-2 md:px-0">
          <div className="flex items-center gap-4">
            <LegendSwatch color={AI_AGENT_COLOR} label={t("Agents")} />
            <LegendSwatch color={AI_CRAWLER_COLOR} label={t("Crawlers")} />
          </div>
          <BucketSelection />
        </div>
        {isLoading ? (
          ...
          <div className="h-[300px] w-full flex items-center justify-center text-neutral-500">
            <div className="text-center">
              <p className="text-lg font-medium">{t("No AI traffic in this period")}</p>
              <p className="text-sm">{t("Try a wider date range, or check the coverage note below.")}</p>
            </div>
          </div>
        ) : (

...
function AiStat({ label, value, hint, isLoading }: { label: string; value: number; hint: string; isLoading: boolean }) {
  return (
      ...
      <CardContent className="p-0">
        <div className="grid grid-cols-2 md:grid-cols-4">
          <AiStat
            ...
            hint={t(
              "Every request from a named AI system: training crawlers, answer-engine indexers, and agents fetching on a person's behalf."
            )}
            isLoading={isLoading}
          />
          <AiStat
            ...
            hint={t(
              "A person asked an assistant to open a page and it fetched it there and then — ChatGPT-User, Claude-User, Perplexity-User and friends. Closer to a visit than to a crawl."
            )}
            isLoading={isLoading}
          />
          <AiStat
            ...
            hint={t(
              "Background crawling: pages collected to train a model or to build an answer-engine index. Nobody is waiting on the other end."
            )}
            isLoading={isLoading}
          />
          <AiStat
            ...
            value={referrals}
            hint={t(
              "People who arrived on your site from an AI product in this period. Compare against crawls to see which operators give something back."
            )}

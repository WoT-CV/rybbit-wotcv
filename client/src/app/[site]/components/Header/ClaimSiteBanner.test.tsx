...
vi.mock("@/lib/configs", () => ({
  useConfigs: () => ({
    configs: {
      ...
      capabilities: { unclaimedSites: state.unclaimedSites, turnstile: state.turnstile },
    },
    isLoading: false,
  }),
}));
...
vi.mock("@/app/signup/components/AccountStep", () => ({
  AccountStep: ({ socialCallbackURL, loginHref, onSubmit, turnstileEnabled, setTurnstileToken }: any) => (
    <div>
      <a href={socialCallbackURL}>OAuth</a>
      <a href={loginHref}>Log in</a>
      <button onClick={onSubmit}>Create account</button>
      {turnstileEnabled && <button onClick={() => setTurnstileToken("captcha-test")}>Complete captcha</button>}
    </div>
  ),
}));
...
describe("claim flow", () => {
  it("does not open onboarding or render its banner when the capability is off", () => {
    state.unclaimedSites = false;
    state.params = new URLSearchParams("claim=1");
    show();
    expect(screen.queryByRole("button", { name: "Claim this site" })).toBeNull();
  ...
  it("does not reopen a dismissed URL-resumed dialog when metadata refreshes", async () => {
    state.params = new URLSearchParams("claim=1");
    show();
    await screen.findByRole("button", { name: "Claim site" });
    fireEvent.click(screen.getByRole("button", { name: "Close" }));
    await waitFor(() => expect(screen.queryByRole("button", { name: "Claim site" })).toBeNull());
    await client.invalidateQueries({ queryKey: ["get-site"] });
    expect(screen.queryByRole("button", { name: "Claim site" })).toBeNull();
  });
  ...
  it("requires configured Turnstile on self-hosted signup and forwards its token", async () => {
    state.cloud = false;
    state.user = null;
    ...
    state.signup.mock.calls[0][1].onRequest({ headers });
    expect(headers.get("x-captcha-response")).toBe("captcha-test");
    await waitFor(() => expect(state.replace).toHaveBeenCalledWith("/7"));
  });
});

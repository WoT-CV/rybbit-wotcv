    ...
    const tableStatsResult = await clickhouse.query({
      query: `
        ...
        WHERE database = currentDatabase() AND active = 1
          AND table IN ('events', 'session_replay_events', 'session_replay_metadata', 'session_replay_metadata_v2')
        GROUP BY table
    ...
    const rowsByDateResult = await clickhouse.query({
      query: `
        SELECT date, table, rows_inserted FROM (
          ...
          GROUP BY date
          UNION ALL
          SELECT toDate(start_time) as date, 'session_replay_metadata_v2' as table, count() as rows_inserted

...
import { createDashboardCache, dashboardCacheNamespace } from "./dashboardCache.js";
...
describe("dashboard response cache", () => {
  it("isolates revisions and fork identity mode even on the same database", () => {
    const env = { CLICKHOUSE_HOST: "http://clickhouse:8123", CLICKHOUSE_DB: "analytics", WOTCV_GIT_SHA: "rev-a" };
    const first = dashboardCacheNamespace(env);
    expect(first).toBe(dashboardCacheNamespace({ ...env }));
    expect(first).toBe(dashboardCacheNamespace({ ...env, IDENTITY_RESOLUTION_V2: "true" }));
    for (const change of [
      { CLICKHOUSE_HOST: "http://other:8123" },
      { CLICKHOUSE_DB: "other" },
      { WOTCV_GIT_SHA: "rev-b" },
      { IDENTITY_RESOLUTION_V2: "false" },
    ])
      expect(dashboardCacheNamespace({ ...env, ...change })).not.toBe(first);
  });

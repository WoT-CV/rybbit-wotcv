...
import { clickhouseResolvedIdentifiedUserId } from "../../../db/clickhouse/identityDictionary.js";
...
export const getSessionFilterStatement = (filters: string | undefined, siteId: number, timeStatement: string) =>
  getFilterStatement(filters ?? "", siteId, timeStatement, {
    ...
    // Session projections already resolve the identity against raw events.
    // The outer filter has no site_id and must not consult the dictionary again.
...
export const buildFilteredSessionsCTE = (
  ...
  for (const { parameter } of parsedFilters) {
    ...
    if (parameter === "user_id") {
      aggregates.add("argMax(events.user_id, timestamp_ms) AS user_id");
      aggregates.add(`argMax(${clickhouseResolvedIdentifiedUserId("events")}, timestamp_ms) AS identified_user_id`);

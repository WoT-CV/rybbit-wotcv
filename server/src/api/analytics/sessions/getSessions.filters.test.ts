...
vi.mock("../../../services/userIdentity/userIdentityService.js", async importOriginal => ({
  ...
  resolveUserIdentity: vi.fn(async (_siteId: number, userId: string) => ({
    canonicalUserId: userId,
    anonymousIds: ["device-a"],
  })),
}));
...
describe("buildSessionsQuery with user and event_name filters", () => {
  it("should scope sessions to the user while filtering by event_name via subquery", async () => {
    const { query } = await buildSessionsQuery(
      {

...
describe("buildEventsQuery filters", () => {
  it("keeps WoT-CV identity resolution in the event projection", () => {
    const { query } = buildEventsQuery(baseQuery(), SITE_ID);

    expect(query).toContain("dictGetOrDefault('user_identity_dict'");
    expect(query).toContain("AS identified_user_id");
  });

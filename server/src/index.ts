...
const authReplayRead = authSiteScoped("replay", "read");
...
server.get("/api/script.js", async (_, reply) =>
  reply.header("Vary", "Origin, Accept-Encoding").sendFile("script.js", { maxAge: "1h" })
);
server.get("/api/replay.js", async (_, reply) =>
  reply.header("Vary", "Origin, Accept-Encoding").sendFile("rrweb.min.js", { maxAge: "1d" })
);
server.get("/api/metrics.js", async (_, reply) =>
  reply.header("Vary", "Origin, Accept-Encoding").sendFile("web-vitals.iife.js", { maxAge: "1d" })
);
...
async function analyticsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/sites/:siteId/growth-accounting", publicAnalyticsRead, getGrowthAccounting);
...
async function sessionReplayRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/sites/:siteId/session-replay/:sessionId/exports/:exportId", authReplayRead, getReplayExportStatus);
  fastify.get(
    "/sites/:siteId/session-replay/:sessionId/exports/:exportId/download",
    authReplayRead,
    downloadReplayExport
  );
  fastify.delete("/sites/:siteId/session-replay/:sessionId/exports/:exportId", authReplayWrite, cancelReplayExport);
}

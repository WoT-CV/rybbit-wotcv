...
export function buildQueryUserStatements(
  database: string,
  user: string,
  password: string,
  mainUser = process.env.CLICKHOUSE_USER || "default"
): string[] {
  ...
  // Provisioning resets grants. A misconfigured username must never revoke
  // the ingest/migration user's permissions or change its credentials.
  if (user === mainUser || user === "default") {
    throw new Error("ClickHouse query user must be separate from the main/default user");
  }
  const profile = user;

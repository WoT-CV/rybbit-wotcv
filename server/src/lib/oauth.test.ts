      ...
      pool: {
        connect: async () => ({
          query: (async (sql: string, parameters: unknown[]) => {
            // Match node-postgres Date serialization (local time + offset).
            // PGlite writes Dates as UTC but reads timestamp-without-zone as
            // local time; without this the harness expires fresh codes in CET.
            const result = await database.query(
              sql,
              parameters.map(value => (value instanceof Date ? DateTime.fromJSDate(value).toISO() : value))
            );
            return {
              rows: result.rows,

...
describe("userIdService", () => {
  ...
  it("keeps a browser-scoped id stable and scopes it to the site", async () => {
    await expect(
      userIdService.generateUserIdFromClientId("browser-123", 42, { saltUserIds: false })
    ).resolves.toBe(createHash("sha256").update("42\0browser-123\0").digest("hex").substring(0, 32));

...
const state = vi.hoisted(() => ({ isCloud: false, unclaimedSites: true }));
vi.mock("../../lib/runtimeCapabilities.js", () => ({
  runtimeCapabilities: {
    get unclaimedSites() {
      return state.unclaimedSites;
    },
  },
}));
const mocks = vi.hoisted(() => ({
  ...
  invalidate: vi.fn(),
  clickhouseCommand: vi.fn(async (_options: { query: string }) => undefined),
...
it("rejects anonymous creation and skips cleanup when disabled", async () => {
  const site = await unclaimed();
  await (client as any).exec("UPDATE sites SET claim_expires_at = '2000-01-01'");
  state.unclaimedSites = false;
  await expect(unclaimed()).rejects.toMatchObject({ code: "unclaimed_sites_disabled", statusCode: 403 });
...
describe("cleanup", () => {
  it("deletes only expired unclaimed sites, keeping legacy org-less and claimed sites", async () => {
    ...
    const commands = mocks.clickhouseCommand.mock.calls.map(call => call[0]);
    expect(commands).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ query: expect.stringContaining("session_replay_events") }),
        expect.objectContaining({ query: expect.stringContaining("session_replay_metadata ") }),
        expect.objectContaining({ query: expect.stringContaining("session_replay_metadata_v2") }),
      ])
    );
  });

...
import { REPLAY_METADATA_V1_TABLE, REPLAY_METADATA_V2_TABLE } from "../replay/replayMetadataMode.js";
...
class SiteConfigurationLifecycle {
  ...
  async createUnclaimed(input: { domain: string }): Promise<SiteRow> {
    if (!runtimeCapabilities.unclaimedSites) {
      throw new SiteLifecycleError("unclaimed_sites_disabled", 403, "Anonymous site creation is disabled");
    }

...
async function loadStorage(
  options: { cloud?: boolean; accountId?: string; accessKey?: string; secretKey?: string; bucket?: string } = {}
) {
  state.isCloud = options.cloud ?? true;
  vi.stubEnv("R2_ACCOUNT_ID", options.accountId ?? "account-123");
...
describe("r2Storage initialization", () => {
  it("enables configured object storage on self-hosted without cloud mode", async () => {
    const storage = await loadStorage({ cloud: false });
    expect(storage.isEnabled()).toBe(true);
    expect(mocks.clientConfigs).toHaveLength(1);
  });

  it("stays disabled without the account ID and performs no storage I/O", async () => {
    const storage = await loadStorage({ cloud: false, accountId: "" });
